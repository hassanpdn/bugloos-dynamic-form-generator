<template>
      ...
      <div class="w-full border-dashed border-gray-300 rounded p-10 flex justify-center border-2">
            ...
            <div class="w-full justify-between grid grid-cols-3 gap-7">
                  ...
                  <div class="flex justify-between items-center mt-5">
                        <check-box class="mb-2" :details="{ label: 'Editable' }" v-model="form.editable" />
                        <check-box class="mb-2" :details="{ label: 'Deletable' }" v-model="form.deletable" />
                        <check-box class="mb-2" :details="{ label: 'Extendible' }" v-model="form.extendible" />
                  </div>

                  <!-- form fields generated by FieldGenerator component -->
                  <field-generator v-if="!swapping" @handleModalState="handleModalState" @input="handleValue($event, component.label)" v-for="(component, index) in form.fields" :details="componentDetails(component, index)" :key="`component-${index}`"></field-generator>  
      ...
      <div class="flex w-full justify-center mt-3">
            <btn v-if="action !== 'Edit'" @click="handleModalState(true)" bgColor="blue" textColor="black" class="self-center font-bold" text="Add Field"/>
...
import { FormDetails } from '@/typings/interface/index';
...
export default defineComponent({
      ...
      methods: {
            componentDetails(component: FormDetails, index: number){
                  let isFirstItem = index === 0;
                  let isLastItem = index === (this.form.fields.length - 1);
                  let isUnique = this.form.fields.length === 1;
                  isUnique && (isFirstItem = isLastItem = false);
                  return {...component, isFirstItem, isLastItem, isUnique};
            },
            ...
            submitForm(){
                  ...
                  if(id){
                        ...
                        forms.push({...this.form, id: crypto.randomUUID()});
                  }
            ...
            handleModalState(state: boolean, action?: string, details?: any){
                  ...
                  */
                  ...
                  if(action === 'edit'){
                        ...
                        this.swapItem(action! , index);
                  }
            },
            swapItem(type: string, index: number) {
                  this.swapping = true;
                  if (type === 'swap-up' && index > 0) {
                        const temp = this.form.fields[index - 1];
                        this.form.fields[index - 1] = this.form.fields[index];
                        this.form.fields[index] = temp;
                  } else if (type === 'swap-down' && index < this.form.fields.length - 1) {
                        const temp = this.form.fields[index + 1];
                        this.form.fields[index + 1] = this.form.fields[index];
                        this.form.fields[index] = temp;
                  }
